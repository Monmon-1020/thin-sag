...
pub fn load_rules() -> Vec<DangerRule> {
    ...
    let yml = std::fs::read_to_string(path)
        .expect("failed to read ~/.thin-sag/policy.yaml");
    ...
    doc["danger_rules"]
        ...
        .iter()
        .map(|v| {
            serde_yaml::from_value(v.clone()).expect("invalid DangerRule entry")
        })
        .collect()
}
...
mod tests {
    ...
    fn test_path_regex_match() {
        ...
        let ev_ok = GuardEvent { pid: 1, path: "/tmp/secret.txt".into() };
        let ev_ng = GuardEvent { pid: 1, path: "/tmp/secret.log".into() };
    ...
    fn test_host_regex_match() {
        ...
        let ev = GuardEvent { pid: 2, path: "api.example.com".into() };
    ...
    fn test_exec_regex_match() {
        ...
        let ev = GuardEvent { pid: 3, path: "rm -rf /Users/alice".into() };
        assert!(rule.matches(&ev));
    }
    ...
    fn test_no_regex() {
        ...
        let ev = GuardEvent { pid: 0, path: "/any".into() };
        assert!(!rule.matches(&ev));
    }
}
